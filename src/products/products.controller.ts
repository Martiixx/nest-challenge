...
export class ProductsController {
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Items per page (max 5)', example: 5 })
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Get statistics of non-deleted products with optional date range' })
  @ApiQuery({ name: 'fromDate', required: false, description: 'Start date (ISO format)', example: '2024-01-01' })
  @ApiQuery({ name: 'toDate', required: false, description: 'End date (ISO format)', example: '2024-12-31' })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      type: 'object',
      properties: {
        percentage: { type: 'number', example: 85.00 },
        ...
        dateRange: {
          type: 'object',
          properties: {
            from: { type: 'string', format: 'date-time', example: '2024-01-01T00:00:00.000Z' },
            to: { type: 'string', format: 'date-time', example: '2024-12-31T00:00:00.000Z' }
          }
        },
        message: { type: 'string', example: '85 out of 100 total products are non-deleted (85.00%) in the specified date range.' }
      }
    }
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiQuery({ name: 'threshold', required: false, description: 'Stock threshold (default: 10)', example: 10 })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      ...
      properties: {
        ...
        message: { type: 'string', example: '12 out of 85 active products have low stock (≤ 10 units) - 15.50%' }
      }
    }
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
